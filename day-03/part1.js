...
function drawLineToNextPoint(startingPoint, nextMovement) {
    const points = [];
    const { updater, distance } = parseMovement(nextMovement);

    for (let i = distance; i > 0; i -= 1) {
        const lastPoint = points[points.length - 1] || startingPoint;
        const nextPoint = updater(lastPoint);
        points.push(nextPoint);
    }
    return points;
}

function getPointsForWire(wire) {
    let wirePoints = [[0, 0]];
    for (let i = 0; i < wire.length; i += 1) {
        const lastPoint = wirePoints[wirePoints.length - 1];
        wirePoints = wirePoints.concat(drawLineToNextPoint(lastPoint, wire[i]));
    }
    return wirePoints;
}

function getIntersectionPoints(wire1, wire2) {
    const wire1Points = getPointsForWire(wire1);
    const wire2Points = getPointsForWire(wire2);
    const intersection = [];

    for (let i = 0; i < wire1Points.length; i += 1) {
        for (let j = 0; j < wire2Points.length; j += 1) {
            const wire1Point = wire1Points[i];
            const wire2Point = wire2Points[j];
            if (wire1Point[0] === wire2Point[0] && wire1Point[1] === wire2Point[1]) {
                if (!(wire1Point[0] === 0 && wire1Point[1] === 0)) {
                    intersection.push(wire1Point);
                }
            }
        }
    }
...
function sortByClosestToOrigin(points) {
    const origin = [0, 0];
    const pointsWithDistances = points.map(point => ({ point, distance: getManhattanDistance(origin, point) }));
    return pointsWithDistances.sort((a, b) => a.distance - b.distance);
}

function solveFinalPuzzle(wire1, wire2) {
    return sortByClosestToOrigin(getIntersectionPoints(wire1, wire2))[0].distance;
}

console.log(solveFinalPuzzle(INPUT_WIRE_1, INPUT_WIRE_2));

module.exports = {
    getManhattanDistance,
    parseDirectionSegment: parseMovement,
    drawLineToNextPoint,
    getPointsForWire,
    updaters,
