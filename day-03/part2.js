...
function drawLineToNextPoint(startingPoint, nextMovement) {
    ...
    for (let i = 1; i <= distance; i += 1) {
        ...
        const nextPoint = updater(lastPoint.point);
        points.push({ point: nextPoint, steps: i + startingPoint.steps });
    }
    return points;
}

function getPointsForWire(wire) {
    let wirePoints = [{ point: [0, 0], steps: 0 }];
...
function getIntersectionPoints(wire1, wire2) {
    ...
    for (let i = 0; i < wire1Points.length; i += 1) {
        for (let j = 0; j < wire2Points.length; j += 1) {
            ...
            if (wire1Point.point[0] === wire2Point.point[0] && wire1Point.point[1] === wire2Point.point[1]) {
                if (!(wire1Point.point[0] === 0 && wire1Point.point[1] === 0)) {
                    intersection.push({ point: wire1Point.point, steps: wire1Point.steps + wire2Point.steps });
                }
            }
        }
    }
...
function sortByFewestCombinedSteps(points) {
    return points.sort((a, b) => a.steps - b.steps);
}

function solveFinalPuzzle(wire1, wire2) {
    return sortByFewestCombinedSteps(getIntersectionPoints(wire1, wire2))[0].steps;
}
